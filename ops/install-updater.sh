    ...
    if curl -fsS http://127.0.0.1:8900/api/system/health | "$NODE_PATH" -e '
      let text=""; process.stdin.on("data", chunk => text += chunk); process.stdin.on("end", () => {
        ...
        const ok = body.code === 0 && data.status === "ok" && data.persistentDataConfigured === true
          ...
          && data.modules.every(item => item.moduleReady && (item.persistence !== "sqlite" || item.databaseConnected));
        process.exit(ok ? 0 : 1);
      });
...
[[ $PUBLIC_ORIGIN == https://* ]] || fail 'public-origin 必须使用 HTTPS。'
[[ $EXPECTED_PUBLIC_KEY_SHA256 =~ ^[0-9a-f]{64}$ ]] || fail 'public-key-sha256 必须是 64 位小写 SHA-256 指纹。'
for command in git getent sed curl openssl readlink stat df systemctl systemd-tmpfiles grep awk seq install runuser useradd userdel groupdel pkill pgrep sha256sum; do
...
grep -Fxq 'HOST=127.0.0.1' "$ENV_FILE" || fail '第二批模板要求 HOST=127.0.0.1。'
grep -Fxq 'PORT=8900' "$ENV_FILE" || fail '第二批模板要求 PORT=8900。'
grep -Fxq 'LENOVO_STORE_DATA_DIR=/var/lib/lenovo-store-operations' "$ENV_FILE" || fail '第二批模板只允许固定外部数据目录 /var/lib/lenovo-store-operations。'
grep -Fxq 'LENOVO_STORE_BACKUP_DIR=/var/backups/lenovo-store-operations' "$ENV_FILE" || fail '第二批模板只允许固定备份目录 /var/backups/lenovo-store-operations。'
for key in LENOVO_STORE_UPDATE_ENABLED LENOVO_STORE_UPDATE_TOKEN LENOVO_STORE_PUBLIC_ORIGIN; do
  grep -q "^${key}=" "$ENV_FILE" && fail "$ENV_FILE 已包含 $key；拒绝覆盖现有安全配置。"
done
...
"$NODE_PATH" -e '
  ...
  if (value.origin !== process.argv[1] || value.protocol !== "https:") throw new Error("public-origin 必须是无路径 HTTPS origin");
' "$PUBLIC_ORIGIN"
"$NODE_PATH" -e '
...
UPDATE_TOKEN=$(openssl rand -base64 48 | tr -d '\n')
cat >>"$ENV_FILE" <<EOF
LENOVO_STORE_UPDATE_ENABLED=true
LENOVO_STORE_UPDATE_TOKEN=$UPDATE_TOKEN
LENOVO_STORE_PUBLIC_ORIGIN=$PUBLIC_ORIGIN
...
if wait_for_health "$VERSION" "$COMMIT" 45; then
  ...
  echo "更新管理员令牌仅显示一次：$UPDATE_TOKEN"
  echo "迁移前 checkout 已按 root:root 0700 保留在：$BOOTSTRAP"
  echo '请立即保存令牌到受控密码管理器，完成故障演练后再人工清理 bootstrap 备份。'
  exit 0
fi
