...
function waitForRequestPublication(handle) {
  const sleeper = new Int32Array(new SharedArrayBuffer(4))
  const deadline = Date.now() + 5000
  while (true) {
    const stats = fs.fstatSync(handle)
    if (stats.nlink === 1) return stats
    assert(stats.nlink === 2, '更新请求硬链接数量无效', 'INVALID_REQUEST_LINKS')
    assert(Date.now() < deadline, '更新请求发布未在时限内完成', 'REQUEST_PUBLICATION_TIMEOUT')
...
function claimRequest(config) {
  ...
  let claimed = false
  try {
    handle = fs.openSync(config.requestPath, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW)
    ...
    stats = waitForRequestPublication(handle)
    assert(stats.uid === config.serviceUid && (stats.mode & 0o077) === 0, '更新请求所有者或权限无效', 'INVALID_REQUEST_PERMISSIONS')
    const pathStats = fs.lstatSync(config.requestPath)
    assert(pathStats.isFile() && !pathStats.isSymbolicLink() && pathStats.dev === stats.dev && pathStats.ino === stats.ino, '更新请求在领取前发生变化', 'REQUEST_CHANGED')
    fs.renameSync(config.requestPath, config.processingPath)
    claimed = true
    fsyncDirectory(path.dirname(config.requestPath))
    fsyncDirectory(path.dirname(config.processingPath))
    const processingStats = fs.lstatSync(config.processingPath)
    assert(processingStats.dev === stats.dev && processingStats.ino === stats.ino && processingStats.nlink === 1, '更新请求领取结果无效', 'REQUEST_CHANGED')
    ...
    if (claimed && fs.existsSync(config.processingPath)) removeFileDurably(config.processingPath)
    throw error
  } finally {
...
async function install(request, config, status) {
  ...
  try {
    ...
    if (switched) {
      ...
      if (!rollbackFailed && fs.existsSync(config.transactionPath)) {
        removeFileDurably(config.processingPath)
        removeFileDurably(config.transactionPath)
      }
    }
