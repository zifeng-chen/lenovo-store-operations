...
- 二维码在打印布局中从原位置下移 12px，屏幕预览按 54mm 工牌宽度等比映射该位移；二维码尺寸缩小为原尺寸的 90%，并增加 0.5px 宽的联想红正方形边框；下方“联想官方体验店”和“请您添加企业微信”两行文字与二维码关联，文字上缘距二维码底部为 10px，屏幕预览同步等比缩放；文字块字号为 1.8mm，并限制在 5.4mm 高的隐藏溢出安全区内；
- 单张工牌成品尺寸固定为 54mm × 85mm，0.1mm 内缩轮廓线作为裁切边界，不进入工牌盒模型；
- A4 横向按 5 列 × 2 行紧密排列，每页最多 10 张；工牌之间无额外间隙，网格为 270mm × 170mm，物理边距左右各 13.5mm、上下各 20mm；
- 打印文档使用 296mm × 209mm 安全页面盒并偏移 0.5mm，避免浏览器毫米舍入将第 10 张挤到下一页；
- 右侧实时预览保持 297:210 的 A4 横向比例，并将 270mm × 170mm 网格及 54mm × 85mm 工牌等比映射到屏幕，10 张工牌可完整显示；预览区域受限时提供滚动，不再静默裁切第二排；
...
7. 员工工牌网格是否无额外间隙，裁切线是否未占用 54mm × 85mm 成品尺寸；
8. 每名员工的打印份数是否可在 1–99 之间设置，人数、总张数和页数统计是否正确；
9. 1 人 × 10 份是否恰好为 1 页，1 人 × 11 份是否为 2 页且第二页只有 1 张，没有额外空白页；
10. 员工工牌背景、姓名、岗位、二维码和裁切边界是否完整，二维码是否可正常扫码；
11. 浏览器打印缩放是否为实际大小（100%），是否关闭页眉页脚；
12. 打印机驱动的纸张尺寸、方向、边距和缩放是否与浏览器一致。

员工工牌的浏览器预览按真实毫米比例生成，打印文档使用 296mm × 209mm 安全页面盒规避浏览器分页舍入，但打印机驱动仍可能执行二次缩放。首次使用时应打印 10 张和 11 张两组样张，确认分页无额外空白页，并测量裁切边界内的宽高是否为 54mm × 85mm；若尺寸不符，应关闭“适合页面”等缩放选项后重新打印。
...
- 联想员工工牌支持多员工录入、每人独立设置 1–99 打印份数、54mm × 85mm 原始背景工牌，以及 A4 横向零间隙 10 张/页预览与打印，不持久化员工信息；
