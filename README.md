...
- OCR 凭据配置和识别历史；
- 销售金额保存、撤销和恢复；
- 今日统计，以及最近 30 天销售额/有效笔数双指标趋势；趋势图使用 ECharts 双 Y 轴，销售额采用实线与面积、笔数采用虚线与菱形点，避免只依赖颜色区分；
- 趋势图支持悬停或触屏查看完整日期、销售额和有效笔数，可切换近 7/14/30 天，拖动底部滑块或按住 Shift 滚轮缩放，并可点击绘图区任意日期查看明细；
