...
当前最新记录：`2026-09-03`，发布 `0.2.2`：付款凭证 OCR 支持识别记录详情、逐条删除、CSV/JSON 导出和每自然月 500 次免费额度的本机用量展示；独立调用账本按真实 OCR endpoint 请求计数，删除历史和恢复旧备份都不会返还次数。
...
- 标签 SKU 使用 11pt 联想红粗体并上移 1mm，字号比 13pt 商品名称小 2pt；行高为 1.05，与商品名称间距为 0.5mm，屏幕预览与实际打印保持一致。
...
- “新增品类”位于商品录入上方的独立品类管理卡片中；名称输入框和“保存品类”按钮始终显示，无需先点击展开，支持 Enter 保存和 Esc 清空，新增成功后会自动选中该品类用于商品录入；
...
- 百度 OCR 自动识别付款金额；
- OCR 结果人工覆盖保护；
- OCR 凭据配置，以及可查看详情、逐条删除和导出 CSV/JSON 的识别记录管理；
- 按 `Asia/Shanghai` 自然月显示每月 500 次免费额度的本机已用、剩余和超额次数；调用账本按实际百度 OCR endpoint 请求计数，access token 获取不计，删除历史不返还次数；
- 销售金额保存、撤销和恢复；
...
页面中的 500 次额度来自本机自 `0.2.2` 起记录的百度 OCR endpoint 调用，用于门店侧估算；同一百度账号在其他系统产生的调用不会出现在这里，最终额度以百度智能云控制台为准。
...
根 `package.json` 的 `version` 是整套产品的唯一发布版本。准备新版本时先更新版本、README、CHANGELOG 和相关文档并提交，再创建完全一致的严格 `vX.Y.Z` tag。例如发布 `0.2.2`：
...
统一维护 API 始终要求同源请求和 `X-Lenovo-Store-Maintenance: 1`。生产环境可选择以下一种模式：配置至少 24 个字符的 `LENOVO_STORE_MAINTENANCE_TOKEN`，由页面发送 `Authorization: Bearer <token>`；或者在未配置令牌时显式设置 `LENOVO_STORE_ALLOW_UNAUTHENTICATED_MAINTENANCE=true`，允许可信局域网客户端免令牌执行统一备份和恢复。两项同时配置时令牌模式优先，仍会强制 Bearer 鉴权；两项都未配置时生产服务拒绝启动。非生产环境未配置令牌和开关时，仍仅允许服务器本机回环请求。页面令牌只保存在当前页面内存，不写入浏览器持久化存储。
...
本项目当前定位为门店内部工具，没有完整的用户账号系统或公网部署配置。其他业务 API 仍依赖可信内网边界；默认 `HOST=0.0.0.0` 会允许局域网访问，请只在可信网络中运行。启用 `LENOVO_STORE_ALLOW_UNAUTHENTICATED_MAINTENANCE=true` 后，任何能访问服务端口并发送固定维护标识的客户端都可能下载或覆盖业务数据库，因此只应在受防火墙保护的可信局域网中使用，禁止在公网环境启用。若需要公网部署，应在反向代理层增加 HTTPS、统一身份认证、访问控制和请求限制；维护令牌也不能替代全站登录系统。
