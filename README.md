...
当前最新记录：`2026-08-20`，新增工作台 GitHub 仓库入口，并建立每次更新同步文档和更新日志的长期维护规范。
...
- 商品和品类管理；
- 桌面端使用占满视口的左右工作台：左侧固定放置搜索、自动换行的多行品类筛选、当前结果全选、商品录入和 JSON 备份恢复，右侧商品列表占满剩余空间并独立滚动，表头始终可见；布局已针对 1920 × 1080 Portal 内嵌区域压缩，常用工具无需滚动即可完整显示；
