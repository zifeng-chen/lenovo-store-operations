...
- 默认工牌按参考设计使用纯白底：上方显示姓名和岗位，下方显示 30mm 二维码及“联想官方体验店”“企业微信”两行文案；
...
- 打印文档使用完整的 297mm × 210mm A4 横向页面，按 5 列 × 2 行紧密排列；网格严格为 270mm × 170mm，工牌之间无间隙，物理边距左右各 13.5mm、上下各 20mm；
- 右侧实时预览保持 297:210 的 A4 横向比例，并将两套主题、270mm × 170mm 网格和 54mm × 85mm 工牌等比映射到屏幕；10 张工牌可完整显示，预览区域受限时提供滚动；
...
员工工牌的浏览器预览按真实毫米比例生成，打印文档使用完整的 297mm × 210mm A4 横向页面，内部 270mm × 170mm 网格严格由 5 × 2 个 54mm × 85mm 裁切格组成。打印机驱动仍可能执行二次缩放；首次使用时应打印 10 张和 11 张两组样张，确认分页无额外空白页，并跨 5 列测量 270mm、跨 2 行测量 170mm。若尺寸不符，应确认纸张为 A4、缩放为 100%，并关闭“适合页面”“缩放到可打印区域”等选项后重新打印。
...
- 联想员工工牌支持多员工录入、每人独立设置 1–99 打印份数及默认工牌/联想红工牌主题，使用真实 54mm × 85mm 裁切格和完整 A4 横向页面完成零间隙 10 张/页预览与打印，不持久化员工信息；
