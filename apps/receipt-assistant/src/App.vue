...
async function loadHistory(page=1){const id=++historyRequestId;historyController?.abort();const controller=new AbortController();historyController=controller;historyLoading.value=true;try{const data=await request(`/ocr/history?page=${page}&pageSize=${history.value.pageSize}`,{signal:controller.signal});if(id===historyRequestId)history.value=data}catch(e){if(id===historyRequestId)ElMessage.error(message(e,'OCR 识别记录加载失败'))}finally{if(id===historyRequestId){historyLoading.value=false;historyController=null}}}
